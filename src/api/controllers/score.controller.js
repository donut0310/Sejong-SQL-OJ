...
export class ScoreController {
  ...
  async scoreing() {
    ...
    let result = 0;
    let sql =
      "select tc_cnt from problem where p_id=? and week_info=? and class_id=? ";
    let sql2 =
      "select tc_content from testcase_problem where p_id=? and week_info=? order by tc_id asc;";
    let sql3 =
      "select tc_answer from testcase_problem where p_id=? and week_info=? order by tc_id asc;";
    //문제 생성시 실행하도록
